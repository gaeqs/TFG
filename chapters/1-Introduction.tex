...
Se  \delODR{les} conoce con el nombre de \textbf{procesadores \textit{MIPS}}\footnote{\newODR{Creado por \emph{MIPS Computer Systems}, en la actualidad \emph{MIPS Technologies}}}  a una familia de microprocesadores que implementan
...
sistemas \delODR{embebidos}\newODR{empotrados}, enrutadores y videoconsolas como la \textit{Nintendo 64} o la \textit{PlayStation 2}. \note{y máquinas de Silicon Graphics; se usaban mucho: si ves documentales de ``así se hizo'' de Star Wars, hasta ``La Amenaza Fantasma'' incluida, los efectos especiales los hacían con máquinas SGI, y salen en los documentales}\\
...
informática con diversas herramientas usad\delODR{o}\newODR{a}s en el desarrollo, construcción y depuración de \textit{software}.
Un \textit{IDE} suele estar desarrollado con dos objetivos clave\delODR{s} en mente: maximizar la productividad del
desarrollador y evitar que este tenga que utilizar herramientas externas en su trabajo.

Existe una gran variedad de entornos de desarrollo\delODR{s} en el mercado.
Estos se pueden clasificar de diferentes maneras.

\textbf{Según su propósito:}
\begin{itemize}
    \item \textbf{Entornos de desarrollo específicos:} son entornos pensados \delODR{en}\newODR{para} el desarrollo en una
    tecnología concreta.
...
Un emulador o simulador \delODR{puede imitar}\newODR{imita a} una arquitectura, permitiendo
ejecutar aplicaciones ajenas a la arquitectura del computador principal\note{Óscar:¿original más que principal? Además, no entiendo aquí lo de principal}.
...
moderna, multiplataforma y expan\delODR{s}\newODR{d}ible.
...
\begin{itemize}
    ...
    \item \delODR{Dentro de las posibilidades del lenguaje de programación seleccionado,
    b}\newODR{B}uscar un \textit{framework} de desarrollo de aplicaciones de escritorio
    que permita desarrollar aplicaciones modernas y de gran
    calidad\newODR{, dentro de las posibilidades del lenguaje de
      programación seleccionado}.\note{Óscar: no tengo claro qué
      quiere decir aquí ``Buscar'' y me pregunto si eso es
      medible. ¿``Seleccionar'' sería una opción viable?}
\end{itemize}
...
\begin{itemize}
    ...
    y modificar los diferentes elementos estáticos de la aplicación, como \delODR{lo son}
    los idiomas, los temas y la configuración\newODR{, por ejemplo}.
\end{itemize}

\newODR{Óscar: Yo creo que esto no hay que decirlo así: podemos comentarlo}Este objetivo está altamente relacionado con los
